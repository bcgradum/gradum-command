...
export const sbFacilities = {
  getAll: () => sb("/facilities?select=*&order=facility_number.asc"),
  getById: (id: number) => sb(`/facilities?id=eq.${id}&select=*&limit=1`).then((r: any[]) => r[0]),
...
export const sbAthletes = {
  ...
  getByFacility: (facilityId: number, filters: Record<string, string> = {}) => {
    let qs = `/athletes?nearest_facility_id=eq.${facilityId}&select=*&order=priority_score.desc`;
    for (const [k, v] of Object.entries(filters)) qs += `&${k}=${v}`;
    return sb(qs);
  },
